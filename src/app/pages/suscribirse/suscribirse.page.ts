...
export class SuscribirsePage implements OnInit {
  ...
  private fillSubscriptionStaticInfo() {
    const advantagesPro = [
      {
        title: 'Tres subsectores',
        description:
          'Permite elegir tres subsectores dentro del mismo sector para tener más visibilidad y llegar a más clientes.',
      },
      {
        title: 'Descripción de 1000 caracteres',
        description:
          'Cuenta todo lo que necesites sobre tu servicio sin límite de caracteres en la descripción.',
      },
      {
        title: 'Ventajas Free',
        description:
          'Incluye todas las ventajas que puedes disfrutar en el plan free.',
      },
      {
        title: 'Recibe pagos de forma fácil',
        description:
          'Recibe y realiza pagos con otros usuarios a través de Febelink.',
      },
    ];
    const advantagesFree = [
      {
        title: 'Publica tus búsquedas',
        description:
          'Realiza todas las búsquedas que necesites, encuentra clientes y recomienda a otros usuarios.',
      },
      {
        title: 'Contacta con usuarios',
        description: 'Puedes hacer y responder ofertas sin limitaciones.',
      },
      {
        title: 'Chat y notificaciones',
        description:
          'Habla con tus potenciales clientes todo lo que necesites y reibe notificaciones para no perderte ninguna oportunidad.',
      },
      {
        title: 'Foto o logo',
        description: 'Puedes poner una imagen de tu empresa o marca personal.',
      },
      {
        title: 'Descripción de 50 caracteres',
        description: 'Perfecta para una breve descripción sobre tu servicio',
      },
    ];
    this.subscriptionDetails.set('febe-pro', {
      description:
        '9,95€/mes + IVA. Sin compromiso, cancelación en cualquier momento.',
      advantages: advantagesPro,
    });
    this.subscriptionDetails.set('febe-prom', {
      description: '',
      advantages: advantagesFree,
    });
  }
  ...
  async swapSubscription(idSelectedSubscription: number) {
    ...
    (await this.api.swapSubscription(idSelectedSubscription)).subscribe(
      async (response) => {
        await this.utilities.saveUserSubscription(response.subscription);
        this.subscription = await this.utilities.getUserSubscription();
        this.utilities.dismissLoading();
