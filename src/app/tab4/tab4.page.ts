...
export class Tab4Page {
  ...
  async submitForm() {
    ...
    if (p.password != null && p.password != '') {
      if (this.comprobarContraseña(p.password, p.passwordConfirmation)) {
        ...
        (await this.api.editarOfertanteYContra(p.name, p.email, p.descripcion, p.telefono, p.direccion, p.provincia, p.localidad, p.sector, p.sub_sector, p.dni, this.base64img, p.password)).subscribe( res => {
        ...
        this.utilities.showToast("Introduce correctamente la confirmación de contraseña");
      }
    } else {
     ...
     this.utilities.showLoading();
      ...
      if (err.status === 422) {
        ...
        //} else {
          ...
          this.utilities.showAlert('Error al editar los datos', `Ocurrieron los siguientes errores: ${cadenaErrores}`);
       // }
      }else {
  ...
  public selectFamily() {
    ...
    if(this.subscription_details === null){
      ...
      if(this.sectoresPerfil.length > this.subscription_details.max_families){
        var firstThree = [];
        for (let index = 0; index < this.subscription_details.max_families; index++) {
          firstThree[index] = this.sectoresPerfil[index];
        }
        this.sectoresPerfil = [];
        this.sectoresPerfil = firstThree;
        this.showSubscription('Solo puedes seleccionar ' + this.subscription_details.max_families + ' sectores con tu suscripción actual');
      }
  ...
  public selectSubfamily() {
    ...
    if(this.subscription_details === null){
      ...
      if(this.subSectoresPerfil.length > this.subscription_details.max_subfamilies){
        var firstThree = [];
        for (let index = 0; index < this.subscription_details.max_subfamilies; index++) {
          firstThree[index] = this.subSectoresPerfil[index];
        }
        this.subSectoresPerfil = [];
        this.subSectoresPerfil = firstThree;
        this.showSubscription('Solo puedes seleccionar ' + this.subscription_details.max_subfamilies + ' subsectores con tu suscripción actual');
      }
  ...
  async obtenerSectoresPerfil() {
    ...
    (await this.api.obtenerSectoresPerfil(this.perfil.id)).subscribe( response => {
  ...
  async obtenerSubSectoresPerfil() {
    ...
    (await this.api.obtenerSubSectoresPerfil(this.perfil.id)).subscribe( response => {
   ...
   */
   async compartirPerfil() {
    ...
    let subject =  this.perfil.name + ", de Febelink quiere que le des una valoración de su perfil!";
  ...
  async enviarRecomendacion(name) {
    ...
    (await this.api.existeUsuario(name)).subscribe(async value => {
      ...
      if (value) {
        let title = this.perfil.name + " de Febelink quiere que recomiendes su perfil!";
        let desc = "Ve a su panel para ver sus valoraciones!";

        (await this.api.enviarNotificacionPedirRecomendacion(title, desc, name)).subscribe( resp => {
  ...
  public provinciasChange(event: { component: IonicSelectableComponent, value: any }):void {
    this.localidades = [];
  ...
  public localidadesChange(event: { component: IonicSelectableComponent, value: any }):void {
  ...
  async obtenerProvincias() {
    ...
    (await this.api.obtenerProvincias()).subscribe( provincias => {
      ...
      if (this.perfil.province_id != null) {
        let prov = this.provincias.filter(x => x.id == this.perfil.province_id);
  ...
  async obtenerLocalidades(id_provincia) {
    ...
    (await this.api.obtenerLocalidades(id_provincia)).subscribe( localidades => {
