...
export class AssistantSearchSvc
{
  ...
  search( text ): Promise<IKeywords>
  {
    return new Promise( async resolve => {
      ...
      if ( text?.length > 2 && this.iKeyWords.selectorEnabled ) {
        ...
        this.callSearch = ( await this.api.searchByKeys( text )).subscribe((keywords) => {
          console.log({ keywords })
          if ( keywords.length !== 0 )
          {
            this.iKeyWords.keys = []
            for (let key of keywords) {
              let item = { name: this.highlight(key.keyword), value: key.keyword, key }
          ...
          console.log({ keys: this.iKeyWords.keys, iKeyWords: this.iKeyWords })
        })
      }
    })
  }
  ...
  detectKeyPressed( event ): Promise<any>
  {
  ...
  async getSectorsByKeys( key: IKeys )
  {
    ...
    this.callSectors = ( await this.api.getSectorsByKeys( key.value )).subscribe( keywords =>
    {
      ...
      if( keywords.main )
        this.OnGotKeys.emit( keywords.main )
